...
def evaluate(data_loader, model, sents, device, num_beams=4):
    model.eval()
    outputs, targets = [], []
...
if args.do_inference:
    ...
    exp_settings = f"Inference:\nDatset={args.dataset}; Train bs={args.train_batch_size}, num_epochs = {args.num_train_epochs},seed = {args.seed}"
    exp_results = f"F1 = {scores['f1']:.4f}, precision = {scores['precision']:.4f}, recall = {scores['recall']:.4f}"
...
#                 # num_return_sequences=1,  # 确保每个输入只生成一个序列
#                 # num_beams=num_beams  # 添加Beam Search参数
#             )
#
#             dec = [model.tokenizer.decode(ids, skip_special_tokens=True) for ids in outs]
#             target = [model.tokenizer.decode(ids, skip_special_tokens=True) for ids in batch["target_ids"]]
